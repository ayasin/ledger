...
export const users = sqliteTable(
	"users",
	{
		...
		is_active: integer("is_active", { mode: "boolean" }).notNull().default(
			true,
		),
...
export const accounts: any = sqliteTable(
	"accounts",
	{
		...
		parent_id: integer("parent_id").references((): any => accounts.id, {
			onDelete: "set null",
		}),
		currency: text("currency").notNull().default("USD"),
		status: text("status", { enum: accountStatuses }).notNull().default(
			"active",
		),
		created_at: integer("created_at", { mode: "timestamp" })
			.notNull()
...
export const transactions = sqliteTable(
	...
	{
		id: integer("id").primaryKey({ autoIncrement: true }),
		transaction_date: integer("transaction_date", { mode: "timestamp" })
			.notNull(),
		...
		memo: text("memo"),
		status: text("status", { enum: transactionStatuses }).notNull().default(
			"draft",
		),
		created_at: integer("created_at", { mode: "timestamp" })
			.notNull()
